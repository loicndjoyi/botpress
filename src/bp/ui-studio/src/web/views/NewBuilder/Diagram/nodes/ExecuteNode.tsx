...
import ActionModalSmall from '../../../FlowBuilder/nodeProps/ActionModalSmall'
...
export class ExecuteWidget extends Component<{ node: ExecuteNodeModel; diagramEngine: any }> {
  handleItemChanged = actionText => {
    ...
    flowBuilder.updateFlowNode({ onReceive: [actionText] })
  }

  render() {
    const node = this.props.node
    const actionText = (node.onReceive && node.onReceive.length && node.onReceive[0]) || ''
